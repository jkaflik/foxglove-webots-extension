...
import ansiUpScript from './dependencies/ansi_up.js?raw';
import assimpScript from './dependencies/assimpjs.js?raw';
import assimpWasmUrl from './dependencies/assimpjs.wasm';
import glmScript from './dependencies/glm-js.min.js?raw';
import quaternionScript from './dependencies/quaternion.min.js?raw';
import libtessScript from './dependencies/libtess.min.js?raw';
import enumScript from './enum.js?raw';
import wrenScript from './wrenjs.js?raw';
import wrenDataUrl from './wrenjs.data';
import wrenWasmUrl from './wrenjs.wasm';
...
export default class WebotsView extends HTMLElement {
  ...
  connectedCallback() {
    ...
    script.textContent = `var Module = window.Module || [];
        window.Module = Module;
        Module['locateFile'] = function(path, prefix) {
        if (path.endsWith('.data'))
          return ${JSON.stringify(wrenDataUrl)};
        if (path.endsWith('.wasm'))
          return ${JSON.stringify(wrenWasmUrl)};
        return prefix + path;
      }`;
  ...
  #init() {
    ...
    window.assimpjs = window.assimpjs || {};
    window.assimpjs.locateFile = path => path.endsWith('.wasm') ? assimpWasmUrl : path;

    promises.push(this.#loadScriptText(ansiUpScript));
    promises.push(this.#loadScriptText(assimpScript));
    promises.push(this.#loadScriptText(glmScript));
    promises.push(this.#loadScriptText(quaternionScript));
    promises.push(this.#loadScriptText(libtessScript));
    promises.push(this.#loadScriptText(enumScript));
    promises.push(this.#loadScriptText(wrenScript));
  }
...
if (window.customElements.get('webots-view') === undefined)
